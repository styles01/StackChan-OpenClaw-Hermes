...
## Context
The initial integration approach was to connect Stack-chan to Rosie via the xiaozhi.me MCP broker:
- Stack-chan connects to xiaozhi.me cloud (as it does by default)
- xiaozhi.me broker connects to our MCP server (`server.py`)
- Our MCP server exposes Rosie tools (rosie_status, rosie_say, rosie_printer_status, etc.)
- The xiaozhi cloud LLM calls our tools when relevant

This works but has fundamental limitations:
- The xiaozhi cloud LLM is the brain, not Rosie — our tools are optional add-ons
...
- The robot's personality is xiaozhi's default, not Rosie's
...
- Rosie IS the brain — her system prompt, her tools, her personality
- Audio pipeline runs through our Gateway (STT → Rosie LLM → TTS)
- The robot is genuinely a node of Rosie, not a xiaozhi device with Rosie tools bolted on
