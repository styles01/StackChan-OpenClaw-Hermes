...
1. **ESP-SR custom model** — generate or train a WakeNet model for "Hey Agent A", compile into firmware
...
## Decision
Generate a custom "Hey Agent A" ESP-SR/WakeNet model and compile it into our firmware. Keep on-device offline wake word detection (no latency, works without network).

## Consequences
- Wake word is "Hey Agent A" — matches the robot's identity
