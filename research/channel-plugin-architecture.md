...
**Recommendation:** Since Stack-chan is a single physical device talking to one agent (Rosie), the HTTP endpoint is the pragmatic fit — use `model: openclaw/rosie`, a stable `user`/`x-openclaw-session-key` per conversation for continuity, and `x-openclaw-message-channel` to give it a channel identity. A full channel plugin is only warranted if Stack-chan needs to behave as a first-class messaging network (its own users/rooms/webhooks/outbound transport) or appear in `channels list`/status.
