...
Research: how the Gateway routes a new channel/device to a specific agent (agent-a) by default, and why `model: openclaw/agent-a` works without a session key but not with one.
...
Canonical session-key shape: `agent:<agentId>:<mainKey>` (e.g. `agent:agent-a:main`, `agent:agent-a:telegram:group:...`).
...
Downstream, `toAgentStoreSessionKey` scopes a bare key into the selected agent's namespace (`stackchan:*` → `agent:agent-a:stackchan:*`).
...
## 4. Why `model: openclaw/agent-a` works without a session key but not with one

- **Without a session key**: the endpoint auto-generates a key in the correct `agent:agent-a:<...>` form via `buildAgentMainSessionKey({ agentId: "agent-a", ... })`. Agent = agent-a (from model), session = correctly namespaced. ✓
- **With `x-openclaw-session-key: stackchan:*`**: the agent is still agent-a (model), but the *raw* session key is returned verbatim. If the downstream store fails to re-scope a bare `stackchan:*` into `agent:agent-a:...` (or the caller expects it to carry an `agent:` prefix to be recognized), the conversation is stored under a mismatched bucket — the agent resolves, but the session does not attach to the expected `agent:agent-a:...` conversation, so it appears to "not work."
...
## 5. Config changes to route `x-openclaw-session-key: stackchan:*` to Agent A

Option 1 — **Don't rely on the session key for agent selection.** Always send `model: openclaw/agent-a` (or header `x-openclaw-agent-id: agent-a`). Then use a session key that is already correctly namespaced for agent-a, e.g.:
- `x-openclaw-session-key: agent:agent-a:stackchan:<device>` — fully-qualified, unambiguous.
- Or reuse a stable OpenAI `user` string (e.g. `user: "stackchan"`), which the endpoint auto-scopes into `agent:agent-a:user:stackchan`. This is the cleanest fix.

No Gateway config change is required — this is purely a request-side convention.

Option 2 — **Make agent-a the default agent.** `agents.list[].default: true` (or first list entry). Then `model: openclaw` (or `openclaw/default`) routes to agent-a with any session key.
...
1. Agent selection: send `model: openclaw/agent-a` or `x-openclaw-agent-id: agent-a`.
2. Session scoping: send `x-openclaw-session-key: agent:agent-a:stackchan:<id>` (fully qualified), or send an OpenAI `user` string and let the Gateway build the scoped key.
...
- Fix: send `model: openclaw/agent-a` + a fully-qualified `agent:agent-a:stackchan:<id>` session key, or use a stable `user` string. No channel plugin required.
